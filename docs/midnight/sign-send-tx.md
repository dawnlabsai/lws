...
The request is routed by its JSON `method` field. **This command documents `balanceUnsealed`**, the
method available here; the maker-side methods (`makeTransfer`, `makeIntent`, `balanceSealedTransaction`)
and the MIP offer paths are added in [Midnight 2.5].
...
mint claim, or a not-yet-supported maker encoding) is rejected with a **precise** error rather than a
generic "unsupported".
